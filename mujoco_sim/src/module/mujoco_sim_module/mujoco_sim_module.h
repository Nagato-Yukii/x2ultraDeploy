...
class MujocoSimModule : public aimrt::ModuleBase {
 public:
  struct Options {
    ...
    std::vector<double> initial_base_pos;

    struct InitialJointPosition {
      std::string name;
      double position = 0.0;
    };
    std::vector<InitialJointPosition> initial_joint_positions;
    std::vector<InitialJointPosition> stand_joint_positions;
