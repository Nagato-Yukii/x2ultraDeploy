...
namespace {
std::string InferFrameId(std::size_t joint_num) {
  switch (joint_num) {
    case 12:
      return "x2_leg";
    case 14:
...
void JointSensorRos2Publisher::Initialize(YAML::Node options_node) {
  ...
  frame_id_ = InferFrameId(joint_num_);

  AIMRT_CHECK_ERROR_THROW(aimrt::channel::RegisterPublishType<aimdk_msgs::msg::JointStateArray>(publisher_),
...
void JointSensorRos2Publisher::PublishSensorData() {
  ...
  executor_.Execute([this, state_array = std::move(state_array)]() {
    aimdk_msgs::msg::JointStateArray joint_state;

    const auto timestamp = std::chrono::duration_cast<std::chrono::nanoseconds>(
                               std::chrono::system_clock::now().time_since_epoch())
                               .count();
    joint_state.header.stamp.sec = static_cast<int32_t>(timestamp / 1000000000LL);
    ...
    for (size_t i = 0; i < joint_num_; ++i) {
      aimdk_msgs::msg::JointState state;
      ...
      state.coil_temp = state_array[i].coil_temp_state;  // 使用来自 .h 的快照默认值
      state.motor_temp = state_array[i].motor_temp_state;
      state.motor_vol = state_array[i].motor_vol_state;
