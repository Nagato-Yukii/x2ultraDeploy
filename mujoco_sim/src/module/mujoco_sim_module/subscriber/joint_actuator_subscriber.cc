...
void JointActuatorRos2Subscriber::Initialize(YAML::Node options_node) {
  ...
  AIMRT_CHECK_ERROR_THROW(aimrt::channel::Subscribe<aimdk_msgs::msg::JointCommandArray>(
                              subscriber_,
                              std::bind(&JointActuatorRos2Subscriber::EventHandle, this, std::placeholders::_1)),
                          "Subscribe failed.");
}
void JointActuatorRos2Subscriber::EventHandle(
    const std::shared_ptr<const aimdk_msgs::msg::JointCommandArray>& commands) {
  ...
  for (size_t ii = 0; ii < joint_num_; ++ii) {
    ...
    if (itr == joint_names_vec_.end()) [[unlikely]] {
      AIMRT_WARN("Invalid msg for topic '{}', msg: {}, Joint name '{}' is not matched.",
                 subscriber_.GetTopic(), aimdk_msgs::msg::to_yaml(*commands), command.name);
